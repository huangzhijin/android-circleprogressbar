...
//                canvas.drawLine(startX, startY, stopX, stopY, mProgressPaint);
                float r=interCircleRadius+mLineWidth/2;
                RectF oval = new RectF(mCenterX - r, mCenterX - r, mCenterX + r, mCenterX + r); // 用于定义的圆弧的形状和大小的界限
                canvas.drawArc(oval, DEFAULT_START_DEGREE, 360.0f * getProgress() / getMax(), false, mProgressPaint);
            } else {
                canvas.drawLine(startX, startY, stopX, stopY, mProgressBackgroundPaint);
            }
        }
       x = mCenterX + (float) Math.sin(2.0f * Math.PI * getProgress() / getMax()) * outerCircleRadius;
       y = mCenterX - (float) Math.cos(2.0f * Math.PI  * getProgress() / getMax()) * outerCircleRadius;
        x2 = mCenterX + (float) Math.sin(2.0f * Math.PI * getProgress() / getMax()) * interCircleRadius;
        y2 = mCenterX - (float) Math.cos(2.0f * Math.PI  * getProgress() / getMax()) * interCircleRadius;

        mProgressPaint.setStrokeWidth(mLineWidth*2);
        canvas.drawPoint((x+x2)/2,(y+y2)/2,mProgressPaint);
    }
